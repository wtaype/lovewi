import{j as i}from"./vendor-gzd0YkcT.js";import{auth as c,db as h}from"./firebase-D9gaOqhL.js";import{q as g,c as $,w,b as y,l as k,d as C}from"./firebase-XkXceBzL.js";import{N as u,g as j,f as D,b as f}from"./widev-CQFaY4xL.js";import{a as T}from"./wii-CmqZrdN0.js";let o=[],l=null;const N=()=>`
...
`,q=async()=>{if(console.log(`✅ Smile de ${T}`),!c.currentUser)return u("Debes iniciar sesión","error"),window.location.hash="#/auth?mode=login";const t=j("wiSmile");i("#smileUser").html(`<i class="fas fa-user"></i> ${t?.usuario||c.currentUser.email} • ${t?.email||c.currentUser.email}`),i(document).on("click.sm","#btnActualizar",p).on("click.sm","#btnCancelar, #modalEliminar",a=>i(a.target).is("#btnCancelar, #modalEliminar")&&v()).on("click.sm","#btnConfirmar",A).on("click.sm",".btn_abrir",a=>window.open(i(a.currentTarget).data("url"),"_blank")).on("click.sm",".btn_copiar",a=>D(i(a.currentTarget).data("url"),a.currentTarget,"¡Copiado!")).on("click.sm",".btn_eliminar_msg",a=>{l=i(a.currentTarget).data("id"),i("#modalEliminar").addClass("show")}),await p()};async function p(){const t=c.currentUser?.uid;if(!t)return;const a=i("#btnActualizar");a.find("i").addClass("fa-spin").end().prop("disabled",!0),E();try{const s=g($(h,"wiLoves"),w("uid","==",t));o=(await y(s)).docs.map(e=>({id:e.id,...e.data()})),o.sort((e,n)=>{const d=e.creado?.toDate?.()||e.creado?.seconds?new Date(e.creado.seconds*1e3):new Date(e.creado||0);return(n.creado?.toDate?.()||n.creado?.seconds?new Date(n.creado.seconds*1e3):new Date(n.creado||0))-d}),_(o),console.log(`📬 ${o.length} mensajes`)}catch(s){console.error("❌",s),i("#smileTableBody").html(`
  ...
  `).join(""))}function _(t){if(!t.length)return i("#smileTableBody").html(`
    ...
    `);const a={amor:"fa-heart",amistad:"fa-user-friends",saludo:"fa-gift",declaracion:"fa-comment-dots",aniversario:"fa-calendar-star",carta:"fa-envelope"};i("#smileTableBody").html(t.map(s=>{const r=s.plantilla||"amor",e=a[r]||"fa-envelope",n=b(s.nombre||s.de||"Sin título"),d=b(s.para||"Alguien especial"),m=`${location.origin}/?${s.id}`;return`
      <tr data-id="${s.id}">
